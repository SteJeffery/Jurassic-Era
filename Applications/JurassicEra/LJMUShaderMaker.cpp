...
// for Actor Objects. 
//////////////////////////////////
MaterialPtr LJMUShaderMaker::MakeActorPhongTex(RendererDX11& prdr)
{
	MaterialPtr tmat = MaterialPtr(new MaterialDX11());
	RenderEffectDX11* tfx = new RenderEffectDX11();

	tfx->SetVertexShader(prdr.LoadShader(VERTEX_SHADER, std::wstring(L"X-LJMUPhongTex.hlsl"), 
		                                                std::wstring(L"VSMAIN"), 
													    std::wstring(L"vs_4_0")));
	tfx->SetPixelShader(prdr.LoadShader(PIXEL_SHADER, std::wstring(L"X-LJMUPhongTex.hlsl"), 
	...
	tfx->m_iBlendState = RendererDX11::Get()->CreateBlendState(&blendConfig);
